...
function boundedInteger(name, fallback, maximum) {
    ...
    if (value > maximum)
        throw new Error(`${name} doit être inférieur ou égal à ${maximum}`);
    return value;
}
export const env = {
...
if (env.trustProxyHops > 5)
    throw new Error('TRUST_PROXY_HOPS doit être inférieur ou égal à 5');
if (env.jwt.accessSecret.length < 32 || env.jwt.refreshSecret.length < 32)
    throw new Error('Les secrets JWT doivent contenir au moins 32 caractères');
