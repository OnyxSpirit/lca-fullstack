...
crmRouter.post('/activities', authorize(...CRM_WRITE), asyncHandler(async (request, response) => { const body = request.body; const leadId = text(body.leadId, 'leadId', 30, true); const lead = await accessibleLead(leadId, request); const type = text(body.type, 'type', 30, true); if (!activityTypes.includes(type))
