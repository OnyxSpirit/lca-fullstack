...
customerRouter.get('/customers', authorize(...READ), asyncHandler(async (request, response) => { const scoped = scope(request); const search = typeof request.query.search === 'string' ? request.query.search.trim() : ''; const term = `%${search}%`; const normalizedPhone = normalizePhone(search); const phoneTerm = `%${normalizedPhone}%`; const type = typeof request.query.type === 'string' ? request.query.type : null; if (type && !customerTypes.includes(type))
