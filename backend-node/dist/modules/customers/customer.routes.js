...
const text = (value, name, max = 255, required = false) => {
    ...
    if (typeof value !== 'string')
        throw new HttpError(400, `${name} doit être une chaîne`);
    const result = value.trim();
    if (!result && required)
        throw new HttpError(400, `${name} est requis`);
    if (result.length > max)
        throw new HttpError(400, `${name} est trop long`);
    if (name === 'email')
        validEmail(result || null);
    if (name === 'phone' || name === 'secondaryPhone')
        validPhone(result || null);
    return result || null;
};
...
export function customerScope(request, permission, alias = 'c', applyFilters = false) {
    ...
    const permissionScope = request.rbac?.isSuperAdmin ? 'GLOBAL' : request.rbac?.permissions.get(permission) ?? undefined;
    if (permissionScope === 'GLOBAL')
        return applyFilters ? { sql: `(? IS NULL OR ${alias}.agency_id=?) AND (? IS NULL OR ${alias}.assigned_user_id=?)`, params: [requestedAgency, requestedAgency, requestedCommercial, requestedCommercial] } : { sql: '1=1', params: [] };
    ...
    if (permissionScope === 'OWN') {
        if (applyFilters && requestedCommercial && requestedCommercial !== request.user.sub)
            throw new HttpError(403, 'Cet utilisateur ne peut consulter que son portefeuille clients');
        return { sql: `${alias}.assigned_user_id=?`, params: [request.user.sub] };
    }
    if (permissionScope === 'CONCESSION')
        return applyFilters ? { sql: `${alias}.agency_id IN (SELECT id FROM agencies WHERE concession_id=(SELECT concession_id FROM agencies WHERE id=?)) AND (? IS NULL OR ${alias}.assigned_user_id=?)`, params: [request.user.agencyId, requestedCommercial, requestedCommercial] } : { sql: `${alias}.agency_id IN (SELECT id FROM agencies WHERE concession_id=(SELECT concession_id FROM agencies WHERE id=?))`, params: [request.user.agencyId] };
    if (permissionScope === 'AGENCY')
        return applyFilters ? { sql: `${alias}.agency_id=? AND (? IS NULL OR ${alias}.assigned_user_id=?)`, params: [request.user.agencyId, requestedCommercial, requestedCommercial] } : { sql: `${alias}.agency_id=?`, params: [request.user.agencyId] };
    throw new HttpError(403, `Scope manquant pour ${permission}`);
}
async function accessibleCustomer(id, request, permission = 'customers.view') { const scoped = customerScope(request, permission); const [row] = await query(`${customerSelect} WHERE c.id=? AND ${scoped.sql}`, [id, ...scoped.params]); if (!row)
    throw new HttpError(404, 'Client introuvable'); return row; }
async function customerById(id) { const [row] = await query(`${customerSelect} WHERE c.id=?`, [id]); if (!row)
    throw new HttpError(404, 'Client introuvable'); return row; }
async function resolveAgency(request, body) { const permissionScope = request.rbac?.isSuperAdmin ? 'GLOBAL' : request.rbac?.permissions.get('customers.create'); const requested = text(body.agencyId, 'agencyId', 30); const agencyId = permissionScope === 'GLOBAL' ? (requested ?? request.user?.agencyId) : request.user?.agencyId; if (!agencyId)
    throw new HttpError(400, 'agencyId est requis'); if (permissionScope === 'CONCESSION' && requested) {
    const [allowed] = await query('SELECT id FROM agencies WHERE id=? AND concession_id=(SELECT concession_id FROM agencies WHERE id=?) AND is_active=TRUE', [requested, request.user?.agencyId]);
    if (!allowed)
        throw new HttpError(403, 'Agence hors concession');
}
else if (permissionScope !== 'GLOBAL' && requested && requested !== request.user?.agencyId)
    throw new HttpError(403, 'Agence hors périmètre'); const [agency] = await query('SELECT id FROM agencies WHERE id=? AND is_active=TRUE', [agencyId]); if (!agency)
...
async function validateAssignee(userId, agencyId, request) { if (!userId)
    return; const scope = request.rbac?.isSuperAdmin ? 'GLOBAL' : request.rbac?.permissions.get('customers.assign'); if (!scope)
    throw new HttpError(403, 'Permission insuffisante : customers.assign'); if (scope === 'OWN' && userId !== request.user?.sub)
...
customerRouter.delete('/customers/:id/contacts/:contactId', requirePermission('customers.update'), asyncHandler(async (request, response) => { const id = routeId(request.params.id), contactId = routeId(request.params.contactId); await accessibleCustomer(id, request, 'customers.update'); const result = await execute('DELETE FROM customer_contacts WHERE id=? AND customer_id=?', [contactId, id]); if (!result.affectedRows)
    throw new HttpError(404, 'Contact introuvable'); response.json({ success: true }); }));
customerRouter.get('/customers/:id/360', requirePermission('customers.view'), asyncHandler(async (request, response) => {
    const id = routeId(request.params.id), customer = await accessibleCustomer(id, request), agencyId = String(customer.agency_id), context = request.rbac;
    const sections = { history: can(context, 'customers.history.view'), crm: can(context, 'crm.prospect.view'), vehicles: can(context, 'vehicles.view'), sales: can(context, 'sales.view'), quotations: can(context, 'quotations.view'), service: can(context, 'service.order.view'), invoices: can(context, 'billing.invoice.view'), payments: can(context, 'billing.payment.view') };
    const contacts = await customer360Query('contacts', id, 'SELECT id,first_name,last_name,role_title,email,phone,is_primary,created_at FROM customer_contacts WHERE customer_id=? ORDER BY is_primary DESC,last_name', [id]);
    const opportunities = sections.crm ? await customer360Query('opportunities', id, 'SELECT o.id,o.title,o.stage,o.expected_value,o.probability,o.expected_close_date,o.lost_reason,o.created_at FROM opportunities o WHERE o.customer_id=? OR o.lead_id IN(SELECT id FROM leads WHERE customer_id=?) ORDER BY o.updated_at DESC', [id, id]) : [];
    const vehicles = sections.vehicles ? await customer360Query('vehicles', id, `SELECT v.id,v.vin,v.registration_number,v.year,v.mileage,v.status,b.name brand,m.name model,ve.name version FROM vehicles v JOIN versions ve ON ve.id=v.version_id JOIN models m ON m.id=ve.model_id JOIN brands b ON b.id=m.brand_id WHERE v.agency_id=? AND (EXISTS(SELECT 1 FROM sale_items si JOIN sales s ON s.id=si.sale_id WHERE si.vehicle_id=v.id AND s.customer_id=?) OR EXISTS(SELECT 1 FROM repair_orders ro WHERE ro.vehicle_id=v.id AND ro.customer_id=?)) ORDER BY v.updated_at DESC`, [agencyId, id, id]) : [];
    const sales = sections.sales ? await customer360Query('sales', id, `SELECT s.id,s.sale_number,s.status,s.total,${sections.payments ? "COALESCE((SELECT i.amount_paid FROM invoices i WHERE i.sale_id=s.id AND i.status<>'cancelled' ORDER BY i.id DESC LIMIT 1),s.deposit_amount)" : "0"} amount_paid,${sections.payments ? "COALESCE((SELECT i.balance_due FROM invoices i WHERE i.sale_id=s.id AND i.status<>'cancelled' ORDER BY i.id DESC LIMIT 1),s.balance_due)" : "0"} balance_due,${sections.payments ? "COALESCE((SELECT i.status FROM invoices i WHERE i.sale_id=s.id AND i.status<>'cancelled' ORDER BY i.id DESC LIMIT 1),'issued')" : "NULL"} financial_status,s.created_at,si.vehicle_id,CONCAT(b.name,' ',m.name,' ',ve.name) vehicle_label FROM sales s LEFT JOIN sale_items si ON si.sale_id=s.id AND si.vehicle_id IS NOT NULL LEFT JOIN vehicles v ON v.id=si.vehicle_id LEFT JOIN versions ve ON ve.id=v.version_id LEFT JOIN models m ON m.id=ve.model_id LEFT JOIN brands b ON b.id=m.brand_id WHERE s.customer_id=? AND s.agency_id=? ORDER BY s.created_at DESC`, [id, agencyId]) : [];
    const quotations = sections.quotations ? await customer360Query('quotations', id, `SELECT q.id,q.quotation_number,q.status,q.total,q.created_at,o.assigned_user_id,CONCAT_WS(' ',owner.first_name,owner.last_name) salesperson_name,CONCAT_WS(' ',creator.first_name,creator.last_name) created_by_name,qi.vehicle_id,CONCAT(b.name,' ',m.name,' ',ve.name) vehicle_label FROM quotations q JOIN opportunities o ON o.id=q.opportunity_id LEFT JOIN users owner ON owner.id=o.assigned_user_id LEFT JOIN users creator ON creator.id=q.created_by LEFT JOIN quotation_items qi ON qi.quotation_id=q.id LEFT JOIN vehicles v ON v.id=qi.vehicle_id LEFT JOIN versions ve ON ve.id=v.version_id LEFT JOIN models m ON m.id=ve.model_id LEFT JOIN brands b ON b.id=m.brand_id WHERE q.customer_id=? AND q.agency_id=? ORDER BY q.created_at DESC`, [id, agencyId]) : [];
    const repairOrders = sections.service ? await customer360Query('repairOrders', id, `SELECT ro.id,ro.order_number,ro.status,ro.complaint,ro.actual_total,ro.created_at,v.id vehicle_id,v.registration_number,CONCAT(b.name,' ',m.name,' ',ve.name) vehicle_label FROM repair_orders ro JOIN vehicles v ON v.id=ro.vehicle_id JOIN versions ve ON ve.id=v.version_id JOIN models m ON m.id=ve.model_id JOIN brands b ON b.id=m.brand_id WHERE ro.customer_id=? AND ro.agency_id=? ORDER BY ro.created_at DESC`, [id, agencyId]) : [];
    const invoices = sections.invoices ? await customer360Query('invoices', id, `SELECT id,invoice_number,status,issue_date,due_date,total,${sections.payments ? 'amount_paid' : '0'} amount_paid,${sections.payments ? 'balance_due' : '0'} balance_due,invoice_type,created_at FROM invoices WHERE customer_id=? AND agency_id=? ORDER BY created_at DESC`, [id, agencyId]) : [];
    const timelineParts = ["SELECT 'customer' event_type,c.id reference_id,CONCAT('Création client ',c.customer_code) title,'Fiche client créée' description,c.created_at event_at FROM customers c WHERE c.id=?"];
    const timelineParams = [id];
    if (sections.crm) {
        timelineParts.push("SELECT 'opportunity',o.id,o.title,CONCAT('Étape ',o.stage),o.created_at FROM opportunities o WHERE o.customer_id=? OR o.lead_id IN(SELECT id FROM leads WHERE customer_id=?)", "SELECT 'activity',a.id,a.subject,a.description,a.created_at FROM activities a WHERE a.customer_id=?");
        timelineParams.push(id, id, id);
    }
    if (sections.sales) {
        timelineParts.push("SELECT 'sale',s.id,CONCAT('Vente ',s.sale_number,' — ',s.status),CONCAT(s.total,' XAF'),s.created_at FROM sales s WHERE s.customer_id=?");
        timelineParams.push(id);
    }
    if (sections.service) {
        timelineParts.push("SELECT 'repair_order',ro.id,CONCAT('OR ',ro.order_number,' — ',ro.status),ro.complaint,ro.created_at FROM repair_orders ro WHERE ro.customer_id=?");
        timelineParams.push(id);
    }
    if (sections.invoices) {
        timelineParts.push("SELECT 'invoice',i.id,CONCAT('Facture ',i.invoice_number,' — ',i.status),CONCAT(i.total,' XAF'),i.created_at FROM invoices i WHERE i.customer_id=?");
        timelineParams.push(id);
    }
    if (sections.payments) {
        timelineParts.push("SELECT 'payment',p.id,CONCAT('Paiement ',p.payment_number,' — ',p.status),CONCAT(p.amount,' XAF'),p.payment_date FROM payments p WHERE p.customer_id=?");
        timelineParams.push(id);
    }
    ...
    const timeline = sections.history ? await customer360Query('timeline', id, `SELECT * FROM (${timelineParts.join(' UNION ALL ')}) events ORDER BY event_at DESC LIMIT 200`, timelineParams) : [];
    response.json({ customer: mapCustomer(customer), sections, contacts, opportunities: opportunities.map(r => ({ id: String(r.id), title: r.title, stage: r.stage, expectedValue: Number(r.expected_value ?? 0), probability: Number(r.probability ?? 0), expectedCloseDate: r.expected_close_date, lostReason: r.lost_reason, createdAt: r.created_at })), vehicles: vehicles.map(r => ({ id: String(r.id), vin: r.vin, registrationNumber: r.registration_number ?? '', brand: r.brand, model: r.model, version: r.version, year: r.year, mileage: Number(r.mileage ?? 0), status: r.status })), sales: sales.map(r => ({ id: String(r.id), saleNumber: r.sale_number, status: r.status, totalSaleTTC: Number(r.total ?? 0), paidAmountTTC: Number(r.amount_paid ?? 0), remainingBalanceTTC: Number(r.balance_due ?? 0), financialStatus: r.financial_status, vehicleId: r.vehicle_id == null ? '' : String(r.vehicle_id), vehicleLabel: r.vehicle_label ?? '', createdAt: r.created_at })), quotations: quotations.map(r => ({ id: String(r.id), quotationNumber: r.quotation_number, status: r.status, total: Number(r.total ?? 0), salespersonId: r.assigned_user_id == null ? null : String(r.assigned_user_id), salespersonName: r.salesperson_name ?? '', createdByName: r.created_by_name ?? '', vehicleId: r.vehicle_id == null ? '' : String(r.vehicle_id), vehicleLabel: r.vehicle_label ?? '', createdAt: r.created_at })), repairOrders: repairOrders.map(r => ({ id: String(r.id), orNumber: r.order_number, status: r.status, symptomsReported: r.complaint ?? '', finalTotalTTC: Number(r.actual_total ?? 0), vehicleId: String(r.vehicle_id), vehicleModel: r.vehicle_label, vehiclePlate: r.registration_number ?? '', createdAt: r.created_at })), invoices: invoices.map(r => ({ id: String(r.id), invoiceNumber: r.invoice_number, status: r.status, issueDate: r.issue_date, dueDate: r.due_date, amountTTC: Number(r.total ?? 0), paidAmountTTC: Number(r.amount_paid ?? 0), remainingAmountTTC: Number(r.balance_due ?? 0), type: r.invoice_type, createdAt: r.created_at })), timeline });
}));
