...
notificationRouter.patch('/notifications/:id/archive', requirePermission('notifications.archive'), asyncHandler(async (request, response) => {
    ...
    const archived = await transaction(async (connection) => {
        const [rows] = await connection.execute(`SELECT id,archived_at FROM notifications WHERE id=? AND user_id=? AND channel='notification' AND deleted_at IS NULL FOR UPDATE`, [notificationId, userId]);
...
notificationRouter.delete('/notifications/:id', requirePermission('notifications.delete'), asyncHandler(async (request, response) => {
    ...
    const deleted = await transaction(async (connection) => {
        const [rows] = await connection.execute(`SELECT id,deleted_at FROM notifications WHERE id=? AND user_id=? AND channel='notification' FOR UPDATE`, [notificationId, userId]);
        const current = rows[0];
        if (!current)
            throw new HttpError(404, 'Notification introuvable');
