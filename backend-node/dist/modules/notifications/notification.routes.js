...
notificationRouter.get('/notifications', asyncHandler(async (request, response) => {
    ...
    const baseWhere = [`user_id=?`, `channel='notification'`], baseParams = [request.user.sub];
...
notificationRouter.get('/notifications/unread-count', asyncHandler(async (request, response) => { const [row] = await query(`SELECT COUNT(*) unreadCount FROM notifications WHERE user_id=? AND channel='notification' AND read_at IS NULL`, [request.user.sub]); response.json({ unreadCount: Number(row?.unreadCount ?? 0) }); }));
...
notificationRouter.patch('/notifications/:id/read', asyncHandler(async (request, response) => { const result = await execute(`UPDATE notifications SET read_at=COALESCE(read_at,NOW()) WHERE id=? AND user_id=? AND channel='notification'`, [routeId(request.params.id), request.user.sub]); if (!result.affectedRows)
    throw new HttpError(404, 'Notification introuvable'); response.json({ success: true }); }));
notificationRouter.delete('/notifications/:id', asyncHandler(async (request, response) => { const result = await execute(`DELETE FROM notifications WHERE id=? AND user_id=? AND channel='notification'`, [routeId(request.params.id), request.user.sub]); if (!result.affectedRows)
    throw new HttpError(404, 'Notification introuvable'); response.json({ success: true }); }));
