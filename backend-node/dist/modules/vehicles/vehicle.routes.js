...
vehicleRouter.get('/vehicles/stats', authorize(...READ), asyncHandler(async (request, response) => {
    ...
    response.json({ total: Number(row?.total ?? 0), ordered: Number(row?.ordered ?? 0), inTransit: Number(row?.in_transit ?? 0), received: Number(row?.received ?? 0), preparation: Number(row?.preparation ?? 0), available: Number(row?.available ?? 0), reserved: Number(row?.reserved ?? 0), sold: Number(row?.sold ?? 0), delivered: Number(row?.delivered ?? 0), dormant: Number(row?.dormant ?? 0), ...(finance ? { stockValue: Number(row?.stock_value ?? 0) } : {}) });
}));
