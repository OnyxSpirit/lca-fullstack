...
const grant = (request, permission) => request.rbac?.isSuperAdmin ? 'GLOBAL' : request.rbac?.permissions.get(permission) ?? undefined;
...
const agency = async (request, permission, requested) => { const scoped = grant(request, permission), current = request.user?.agencyId; if (scoped === 'GLOBAL')
    return requested ? String(requested) : current; if (scoped === 'CONCESSION' && requested) {
    const [row] = await query('SELECT id FROM agencies WHERE id=? AND concession_id=(SELECT concession_id FROM agencies WHERE id=?) AND is_active=TRUE', [String(requested), current]);
    if (!row)
        throw new HttpError(403, 'Agence hors concession');
    return String(requested);
} if (requested && String(requested) !== String(current))
    throw new HttpError(403, 'Agence hors périmètre'); return current; };
...
vehicleRouter.use('/vehicles', (request, _response, next) => {
    ...
    if (!['ordered', 'in_transit', 'received'].includes(initialStatus))
        return next(new HttpError(409, 'Un véhicule doit entrer en stock comme commandé, en transit ou réceptionné.'));
...
async function accessible(id, request, permission = 'vehicles.view') { const scoped = vehicleScope(request, permission); const [row] = await query(`${baseSelect} WHERE v.id=? AND v.archived_at IS NULL AND ${scoped.sql}`, [id, ...scoped.params]); if (!row)
...
vehicleRouter.get('/vehicles/filter-options', requirePermission('vehicles.view'), asyncHandler(async (request, response) => {
    ...
    response.json({ brands: brands.map(row => ({ id: String(row.id), name: String(row.name) })), models: models.map(row => ({ id: String(row.id), brandId: String(row.brand_id), name: String(row.name) })) });
}));
...
vehicleRouter.get('/vehicles/:id', requirePermission('vehicles.view'), asyncHandler(async (request, response) => response.json(mapVehicle(await accessible(idOf(request.params.id), request), hasFinance(request)))));
vehicleRouter.get('/vehicles/:id/360', requirePermission('vehicles.view'), asyncHandler(async (request, response) => {
    const id = idOf(request.params.id), vehicle = await accessible(id, request, 'vehicles.view'), context = request.rbac;
    const sections = { sales: can(context, 'sales.view'), customers: can(context, 'customers.view'), service: can(context, 'service.order.view'), deliveries: can(context, 'delivery.view'), documents: can(context, 'ged.view'), financials: can(context, 'vehicles.financials.view') };
    const [images, features, statusHistory, movements] = await Promise.all([
        ...
        query(`SELECT vm.*,fl.name from_location_name,tl.name to_location_name,fa.name from_agency_name,ta.name to_agency_name,CONCAT_WS(' ',u.first_name,u.last_name) performed_by_name FROM vehicle_movements vm LEFT JOIN locations fl ON fl.id=vm.from_location_id LEFT JOIN locations tl ON tl.id=vm.to_location_id LEFT JOIN agencies fa ON fa.id=vm.from_agency_id LEFT JOIN agencies ta ON ta.id=vm.to_agency_id LEFT JOIN users u ON u.id=vm.performed_by WHERE vm.vehicle_id=? ORDER BY vm.moved_at DESC`, [id]),
    ]);
    const sales = sections.sales ? await query(`SELECT s.id,s.sale_number,s.customer_id,s.status,s.total,s.created_at,${sections.customers ? "CONCAT_WS(' ',c.first_name,c.last_name)" : "NULL"} customer_name FROM sale_items si JOIN sales s ON s.id=si.sale_id ${sections.customers ? 'JOIN customers c ON c.id=s.customer_id' : ''} WHERE si.vehicle_id=? ORDER BY s.created_at DESC`, [id]) : [];
    const reservations = sections.sales ? await query(`SELECT r.id,r.status,r.reserved_at,r.expires_at,r.customer_id,${sections.customers ? "CONCAT_WS(' ',c.first_name,c.last_name)" : "NULL"} customer_name FROM reservations r ${sections.customers ? 'JOIN customers c ON c.id=r.customer_id' : ''} WHERE r.vehicle_id=? ORDER BY r.reserved_at DESC`, [id]) : [];
    const repairOrders = sections.service ? await query('SELECT id,order_number,customer_id,status,complaint,created_at FROM repair_orders WHERE vehicle_id=? ORDER BY created_at DESC', [id]) : [];
    const deliveries = sections.deliveries ? await query('SELECT id,delivery_number,status,scheduled_at,delivered_at FROM deliveries WHERE vehicle_id=? ORDER BY created_at DESC', [id]) : [];
...
vehicleRouter.patch('/vehicles/:id', requirePermission('vehicles.update'), asyncHandler(async (request, response) => {
    ...
    response.json(mapVehicle(await accessible(id, request, 'vehicles.update'), hasFinance(request)));
}));
...
export function assertManualVehicleTransition(current, next) { if (['reserved', 'sold', 'delivered'].includes(next))
    throw new HttpError(409, `Le statut ${next} doit être produit par son workflow métier`); if (!transitions[current]?.includes(next))
    throw new HttpError(409, `Transition ${current} → ${next} interdite`); }
...
vehicleRouter.patch('/vehicles/:id/status', requirePermission('vehicles.status.update'), asyncHandler(async (request, response) => { const id = idOf(request.params.id); const row = await accessible(id, request, 'vehicles.status.update'); const next = txt(request.body.status); if (!DB_STATUSES.includes(next))
    throw new HttpError(400, 'Statut invalide'); assertManualVehicleTransition(String(row.status), next); const reason = optional(request.body.reason); if (['available'].includes(next) && row.status === 'reserved' && !reason)
    throw new HttpError(400, 'Un motif est obligatoire pour libérer une réservation'); await transaction(async (connection) => { await connection.execute('UPDATE vehicles SET status=? WHERE id=?', [next, id]); await connection.execute('INSERT INTO vehicle_status_history(vehicle_id,old_status,new_status,changed_by,reason) VALUES(?,?,?,?,?)', [id, row.status, next, request.user.sub, reason]); }); await notify(request, id, String(row.agency_id), 'vehicles:status-changed', 'Statut véhicule modifié', `${row.stock_number}: ${row.status} → ${next}`); response.json(mapVehicle(await accessible(id, request, 'vehicles.status.update'), hasFinance(request))); }));
vehicleRouter.post('/vehicles/:id/transfer', requirePermission('vehicles.assign_agency'), asyncHandler(async (request, response) => { const id = idOf(request.params.id); const row = await accessible(id, request, 'vehicles.assign_agency'); const toAgencyId = String(integer(request.body.toAgencyId, 'Agence', true)); await assertTransferTarget(request, toAgencyId); const toLocationId = integer(request.body.toLocationId, 'Emplacement'); const reason = txt(request.body.reason, 255); if (!reason)
