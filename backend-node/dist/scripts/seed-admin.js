...
try {
    ...
    const [group] = await connection.execute(`INSERT INTO groups_company(name,code) VALUES('LCA Groupe','LCA')
     ON DUPLICATE KEY UPDATE id=LAST_INSERT_ID(id)`);
    const [concession] = await connection.execute(`INSERT INTO concessions(group_id,name,code,country) VALUES(?,'LCA Concession','LCA-CG','Congo')
     ON DUPLICATE KEY UPDATE id=LAST_INSERT_ID(id)`, [group.insertId]);
    const [agency] = await connection.execute(`INSERT INTO agencies(concession_id,name,code,city,is_active) VALUES(?,'Agence principale','LCA-BZV','Brazzaville',TRUE)
     ON DUPLICATE KEY UPDATE id=LAST_INSERT_ID(id)`, [concession.insertId]);
    await connection.execute(`INSERT INTO settings(scope_type,scope_id,setting_key,setting_value,description) VALUES
    ...
    const [user] = await connection.execute(`INSERT INTO users(agency_id,first_name,last_name,email,password_hash,job_title,is_active)
     VALUES(?,'Super','Administrateur',?,?,'Super Administrateur',TRUE)
     ON DUPLICATE KEY UPDATE id=LAST_INSERT_ID(id),password_hash=VALUES(password_hash),is_active=TRUE`, [agency.insertId, email, hash]);
