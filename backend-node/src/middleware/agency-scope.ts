...
export function enforceAgencyScope(request: Request, _response: Response, next: NextFunction) {
  ...
  // Users, RH et Showroom valident l'agence cible avec le scope de la
  ...
  if(request.path==='/users'||request.path.startsWith('/users/')||request.path==='/hr'||request.path.startsWith('/hr/')||request.path==='/showroom'||request.path.startsWith('/showroom/'))return next();
