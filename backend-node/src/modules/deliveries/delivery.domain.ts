...
export function assertChecklistTemplateAgencyAccess(scope:PermissionScope|null|undefined,actorAgencyId:string|null,targetAgencyId:string|null,sameConcession=false){
  if(scope==='GLOBAL')return;
  if(targetAgencyId==null)throw new HttpError(403,'Un modèle global exige le scope GLOBAL');
  if(!actorAgencyId)throw new HttpError(403,'Aucune agence associée');
  if(scope==='AGENCY'&&targetAgencyId===actorAgencyId)return;
  if(scope==='CONCESSION'&&sameConcession)return;
  if(scope==='OWN')throw new HttpError(403,'Le scope OWN ne permet pas de gérer les modèles de checklist');
  throw new HttpError(403,'Agence hors du scope delivery.checklist.manage');
}
