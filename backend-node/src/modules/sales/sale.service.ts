...
export async function updateStatus(saleId:string,value:unknown,reasonValue:unknown,request:Request,permission:'sales.confirm'|'sales.cancel'){
  ...
  const result=await transaction(async connection=>{
    ...
    if(status==='cancelled'){
      ...
      const[invoices]=await connection.execute<RowDataPacket[]>("SELECT amount_paid FROM invoices WHERE sale_id=? AND status<>'cancelled' FOR UPDATE",[saleId]);if(invoices.reduce((sum,row)=>sum+Number(row.amount_paid??0),0)>.001)throw new HttpError(409,'Une vente ayant reçu un paiement ne peut plus être annulée');
