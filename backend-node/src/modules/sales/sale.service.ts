...
const hidePaymentDetails=(row:RowDataPacket,request:Request):any=>{const result={...row,financially_cleared:row.invoice_id!=null&&String(row.invoice_status)==='paid'&&Number(row.invoice_balance_due)<=.001};return request.rbac?.isSuperAdmin||request.rbac?.permissions.has('billing.payment.view')?result:{...result,invoice_amount_paid:null,invoice_balance_due:null,invoice_status:null,deposit_amount:0,balance_due:0}};
...
export async function create(body:unknown,request:Request){
  ...
  const result=await transaction(async connection=>{
    ...
    await audit(connection,request,saleId,'sale.created',null,{saleNumber,agencyId,customerId:input.customerId,vehicleId:input.vehicleId,salespersonId:requestedSalesperson,...totals});return{saleId,duplicate:false,agencyId,saleNumber,opportunityId:opportunity==null?null:String(opportunity.id)};
  });
  if(!result.duplicate){emitToAgency(result.agencyId,'sales:created',{id:result.saleId,saleNumber:result.saleNumber,vehicleId:input.vehicleId});if(result.opportunityId)await publishCrmLeadUpdated({opportunityId:result.opportunityId,changeType:'sale',customerChanged:true});await notifyPermissions({agencyId:result.agencyId,permissions:['sales.view'],excludeUserIds:[request.user!.sub],subject:'Nouvelle vente',message:`La vente ${result.saleNumber} a été créée.`,eventType:'sale.created',referenceType:'sale',referenceId:result.saleId,eventKey:`sale.created:${result.saleId}`})}
...
export async function updateStatus(saleId:string,value:unknown,reasonValue:unknown,request:Request,permission:'sales.confirm'|'sales.cancel'){
  ...
  const result=await transaction(async connection=>{
    ...
    if(status==='preparation')await lockActiveSaleInvoice(connection,saleId,'preparation');
    if(status==='ready_for_delivery')await lockActiveSaleInvoice(connection,saleId,'ready_for_delivery');
