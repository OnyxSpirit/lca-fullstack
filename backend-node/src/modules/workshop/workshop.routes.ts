...
workshopRouter.patch(
  ...
  asyncHandler(async (r, res) => {
    ...
    await transaction(async (c) => {
      ...
      if(next==='cancelled')await assertWorkNotStarted(c,id);
      if(next==='quality_control'){
        const[actualBillable]=await c.execute<RowDataPacket[]>("SELECT (SELECT COUNT(*) FROM repair_order_estimate_items WHERE repair_order_id=?) expected_items,(SELECT COUNT(*) FROM repair_order_estimate_items e WHERE e.repair_order_id=? AND EXISTS(SELECT 1 FROM repair_order_items roi WHERE roi.estimate_item_id=e.id AND roi.status='active')) confirmed_items,(SELECT COUNT(*) FROM repair_order_items WHERE repair_order_id=? AND status='active') active_actual_items",[id,id,id]);
        const billable=actualBillableState(Number(actualBillable[0]?.expected_items??0),Number(actualBillable[0]?.confirmed_items??0),Number(actualBillable[0]?.active_actual_items??0));
        if(!billable.confirmed)throw new HttpError(409,'Complétez et validez le réel facturable avant de passer au contrôle qualité');
        await assertWarrantyReadyForQuality(c,id);
      }
      if(next==='received')await assertWarrantyDecisionMade(c,id);
      if(next==='in_progress'){await assertCustomerDidNotReject(c,id);await assertWarrantyMayStart(c,id);}
      if(next==='cancelled'){
...
workshopRouter.post(
  ...
  asyncHandler(async (r, res) => {
    ...
    await transaction(async c=>{
      ...
      await assertWarrantyDecisionMade(c,id);
...
workshopRouter.post(
  ...
  asyncHandler(async (r, res) => {
    ...
    const rejected=!Boolean(r.body.approved);const cancelledScheduleIds=await transaction(async c=>{const[orders]=await c.execute<RowDataPacket[]>('SELECT status,agency_id FROM repair_orders WHERE id=? FOR UPDATE',[id]);if(orders[0]?.status!=='waiting_approval')throw new HttpError(409,'La décision client n’est plus disponible');await assertWorkNotStarted(c,id);await assertWarrantyDecisionMade(c,id);const[existing]=await c.execute<RowDataPacket[]>('SELECT id FROM repair_approvals WHERE repair_order_id=? LIMIT 1 FOR UPDATE',[id]);if(existing[0])throw new HttpError(409,'La décision client a déjà été enregistrée');const[estimate]=await c.execute<RowDataPacket[]>('SELECT line_total,tax_rate FROM repair_order_estimate_items WHERE repair_order_id=? FOR UPDATE',[id]);const approvedAmount=estimate.reduce((total,item)=>total+Number(item.line_total)*(1+Number(item.tax_rate)/100),0);if(approvedAmount<=0)throw new HttpError(409,'Un chiffrage positif est obligatoire avant la décision client');await c.execute(
    ...
    );if(rejected){const scheduleIds=await cleanupAbandonment(c,id,String(orders[0].agency_id),r.user!.sub);await c.execute("UPDATE repair_orders SET status='cancelled',cancellation_reason=? WHERE id=?",[txt(r.body.notes,'Motif',5000)||'Travaux refusés par le client',id]);await c.execute("INSERT INTO repair_order_status_history(repair_order_id,old_status,new_status,reason,changed_by)VALUES(?,'waiting_approval','cancelled',?,?)",[id,txt(r.body.notes,'Motif',5000)||'Travaux refusés par le client',r.user!.sub]);return scheduleIds;}return[];});
    for(const scheduleId of cancelledScheduleIds)emitToAgency(String(ro.agency_id),'workshop:schedule-cancelled',{id:scheduleId});if(rejected)emitToAgency(String(ro.agency_id),'parts:stock-changed',{repairOrderId:id,agencyId:String(ro.agency_id)});
...
workshopRouter.post(
  ...
  asyncHandler(async (r, res) => {
    ...
    await transaction(async (c) => {
      ...
      await invalidateWarrantyAllocation(c,id,r.user!.sub);
    });
...
workshopRouter.patch(
  ...
  asyncHandler(async (r,res)=>{
    ...
    const changed=await transaction(async c=>{
      ...
      if(status==='consumed')await invalidateWarrantyAllocation(c,id,r.user!.sub);
...
workshopRouter.post(
  ...
  asyncHandler(async (r,res)=>{
    ...
    const invoiceId=await transaction(async c=>{
      ...
      const [actualItems]=await c.execute<RowDataPacket[]>("SELECT i.*,COALESCE(a.manufacturer_share_ht,0) manufacturer_share_ht FROM repair_order_items i LEFT JOIN repair_order_warranty_allocations a ON a.repair_order_item_id=i.id WHERE i.repair_order_id=? AND i.status='active' FOR UPDATE",[id]);
      const[warranties]=await c.execute<RowDataPacket[]>("SELECT * FROM repair_order_warranties WHERE repair_order_id=? FOR UPDATE",[id]),warranty=warranties[0];
      if(warranty?.decision_status==='PENDING')throw new HttpError(409,'La décision garantie est encore en attente');
      if(warranty?.decision_status==='APPROVED'&&warranty.allocation_status!=='CONFIRMED')throw new HttpError(409,'La ventilation garantie doit être confirmée avant facturation');
      const allocated=warranty?.decision_status==='APPROVED';
      const split=actualItems.map(item=>{const line=Number(item.line_total),rate=Number(item.tax_rate),manufacturerHt=allocated?Number(item.manufacturer_share_ht):0;if(manufacturerHt<0||manufacturerHt>line+.001)throw new HttpError(409,'Ventilation garantie incohérente');const lineTax=money(line*rate/100),manufacturerTax=line?money(lineTax*manufacturerHt/line):0;return{item,line,rate,lineTax,manufacturerHt,manufacturerTax,customerHt:money(line-manufacturerHt),customerTax:money(lineTax-manufacturerTax)}});
      const subtotal=money(split.reduce((s,x)=>s+x.customerHt,0)),tax=money(split.reduce((s,x)=>s+x.customerTax,0)),total=money(subtotal+tax),realTotal=money(split.reduce((s,x)=>s+x.line+x.lineTax,0)),manufacturerSubtotal=money(split.reduce((s,x)=>s+x.manufacturerHt,0)),manufacturerTax=money(split.reduce((s,x)=>s+x.manufacturerTax,0)),manufacturerTotal=money(manufacturerSubtotal+manufacturerTax),no=await nextDocumentNumber(c,'FAC',String(ro.agency_id));
      ...
      for(const x of split.filter(x=>x.customerHt>.001))await c.execute("INSERT INTO invoice_items(invoice_id,repair_order_item_id,part_id,description,quantity,unit_price,discount,tax_rate,tax_amount,line_total)VALUES(?,?,?,?,1,?,0,?,?,?)",[inv.insertId,x.item.id,x.item.part_id,x.item.description,x.customerHt,x.rate,x.customerTax,x.customerHt]);
      if(total<=.001)await c.execute("INSERT INTO invoice_items(invoice_id,description,quantity,unit_price,discount,tax_rate,tax_amount,line_total)VALUES(?,'Récapitulatif atelier — prise en charge constructeur intégrale',1,0,0,0,0,0)",[inv.insertId]);
      if(allocated&&manufacturerTotal>.001){if(!warranty.provider_id)throw new HttpError(409,'Constructeur de garantie obligatoire');const claimNo=await nextDocumentNumber(c,'GAR',String(ro.agency_id));const[claim]=await c.execute<ResultSetHeader>("INSERT INTO warranty_claims(claim_number,warranty_id,provider_id,agency_id,status,subtotal,tax_total,total,balance_due,currency_code,reference,created_by)VALUES(?,?,?,?, 'issued',?,?,?,?,?,?,?)",[claimNo,warranty.id,warranty.provider_id,ro.agency_id,manufacturerSubtotal,manufacturerTax,manufacturerTotal,manufacturerTotal,businessConfig.currencyCode,warranty.authorization_reference,r.user!.sub]);for(const x of split.filter(x=>x.manufacturerHt>.001))await c.execute("INSERT INTO warranty_claim_items(claim_id,repair_order_item_id,description,item_type,quantity_snapshot,real_line_total_ht,tax_rate_snapshot,manufacturer_share_ht,manufacturer_tax,manufacturer_total)VALUES(?,?,?,?,?,?,?,?,?,?)",[claim.insertId,x.item.id,x.item.description,x.item.item_type,x.item.quantity,x.line,x.rate,x.manufacturerHt,x.manufacturerTax,money(x.manufacturerHt+x.manufacturerTax)]);}
      await c.execute("UPDATE repair_orders SET status='invoiced',actual_total=? WHERE id=?",[realTotal,id]);
