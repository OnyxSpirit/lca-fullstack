...
workshopRouter.get('/workshop/config',workshopAuthorize('workshop.view'),asyncHandler(async(r,res)=>{const config=await getEffectiveBusinessSettings(workshopAgencyFromQuery(r));res.json({vatRate:config.vatRate,rates:config.rates,currencyCode:config.currencyCode})}));
...
workshopRouter.get('/repair-orders/:id/labor-rates',serviceAccess('service.order.view'),asyncHandler(async(r,res)=>{
  const ro=await one(idOf(r.params.id),r),config=await getEffectiveBusinessSettings(String(ro.agency_id));res.json({rates:config.rates,currencyCode:config.currencyCode});
}));
...
workshopRouter.post(
  ...
  asyncHandler(async (r,res)=>{
    ...
    await transaction(async c=>{
      ...
      if(key){const[existing]=await c.execute<RowDataPacket[]>('SELECT id,item_type,part_id,quantity FROM repair_order_estimate_items WHERE repair_order_id=? AND request_key=? FOR UPDATE',[id,key]);if(existing[0]){if(existing[0].item_type!==type||String(existing[0].part_id??'')!==String(r.body.partId??'')||Number(existing[0].quantity)!==quantity)throw new HttpError(409,'Clé de chiffrage déjà utilisée avec un contenu différent');return}}
      let partId:null|string=null,description:string,unitPrice:number;
      if(type==='part'){
        ...
        const rateCode=String(r.body.rateCode??'').toUpperCase();if(!['T1','T2','T3','T4'].includes(rateCode))throw new HttpError(400,'Code tarif atelier invalide');
        description=txt(r.body.description,'Description',255,true)!;unitPrice=config.rates[rateCode as keyof typeof config.rates];
      }
      ...
      await c.execute("INSERT INTO repair_order_estimate_items(repair_order_id,part_id,item_type,description,quantity,unit_price,discount,tax_rate,line_total,request_key,created_by)VALUES(?,?,?,?,?,?,?,?,?,?,?)",[id,partId,type,description,quantity,unitPrice,discount,config.vatRate,lineTotal,key,r.user!.sub]);
...
workshopRouter.post(
  ...
  asyncHandler(async (r, res) => {
    ...
    let price=type==='labor'&&estimateItemId?0:type==='labor'&&rateCode?businessConfig.rates[rateCode as keyof typeof businessConfig.rates]:type==='part'?0:Number(r.body.unitPrice),description=txt(r.body.description,"Description",255,!estimateItemId);
    if (
    ...
    await transaction(async (c) => {
      ...
      await c.execute(
        "INSERT INTO repair_order_items(repair_order_id,part_id,part_stock_id,intervention_id,estimate_item_id,item_type,description,quantity,unit_price,discount,tax_rate,line_total,request_key)VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",
        [
          id,
          r.body.partId ?? null,
          partStockId,
