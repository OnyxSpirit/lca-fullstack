...
async function detail(id: string, r: Request): Promise<any> {
  ...
  const [
    ...
    query<RowDataPacket[]>("SELECT id,invoice_number,status,subtotal,tax_total,total,amount_paid,balance_due,issue_date FROM invoices WHERE repair_order_id=? AND status<>'cancelled' ORDER BY id DESC",[id]),
  ]);
  return {
    ...ro,
    items,
