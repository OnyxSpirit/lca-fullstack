...
export async function bootstrapDatabase(options:{faultHook?:MigrationFaultHook}={}){
  ...
  try{
    ...
    if(state==='EMPTY'){
      ...
      await executeResilientMigration(connection,{version:FRESH_BASELINE_VERSION,name:'baseline_001_040+system_seed',sql:baselineSql},options.faultHook,false);
    }
    await ensureMigrationStepJournal(connection);
    if(state==='VERSIONED'&&tableNames.includes('schema_migration_steps')){
      const[baselineSteps]=await connection.execute<RowDataPacket[]>(`SELECT COUNT(*) total,SUM(status='APPLIED') applied FROM schema_migration_steps WHERE version=? AND migration_name='baseline_001_040+system_seed'`,[FRESH_BASELINE_VERSION]);
    ...
    for(const row of versions){
      const version=Number(row.version),name=String(row.name);if(name==='baseline_001_040')continue;
