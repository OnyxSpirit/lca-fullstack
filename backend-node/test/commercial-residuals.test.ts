...
let capturedSalespersonId:string|null=null;
...
before(()=>{
  (pool as any).execute=async(sql:string,params:unknown[]=[])=>{
    ...
    if(sql.includes('FROM sales s')&&sql.includes('WHERE s.id=?'))return[[{id:'70',sale_number:'V-70',agency_id:'1',customer_id:'30',salesperson_id:capturedSalespersonId}],[]];
    return[[],[]];
  };
  (pool as any).getConnection=async()=>({beginTransaction:async()=>{},commit:async()=>{},rollback:async()=>{},release:()=>{},execute:async(sql:string,params:unknown[]=[])=>{
    if(sql.includes('SELECT q.*')&&sql.includes('FOR UPDATE'))return[[quote],[]];
...
beforeEach(()=>{quote={...quote,agency_id:'1',created_by:'200',created_by_name:'Manager Un',commercial_owner_id:'100',commercial_owner_name:'Agent A'};capturedSalespersonId=null});
