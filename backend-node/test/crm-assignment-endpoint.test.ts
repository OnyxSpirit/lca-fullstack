...
let users:Record<string,Fixture>={},created:{assignedUserId:string|null;createdBy:string}|null=null,nextId=100;
...
before(()=>{
  ...
  (pool as any).getConnection=async()=>({beginTransaction:async()=>{},commit:async()=>{},rollback:async()=>{},release:()=>{},execute:async(sql:string,params:unknown[]=[])=>{
    ...
    if(sql.includes('INSERT INTO opportunities('))return[{insertId:nextId+1},[]];
    return[{affectedRows:1},[]];
  }});
});
after(()=>{(pool as any).execute=originalExecute;(pool as any).getConnection=originalGetConnection});
beforeEach(()=>{users={};created=null;nextId++});
...
test('RECEPTIONIST crée sans commercial et reste distinct du propriétaire',async()=>{
  ...
  assert.equal(response.status,201);assert.equal(response.body.assignedUserId,null);assert.notEqual(response.body.assignedUserId,'10');assert.equal(response.body.createdById,'10');
});
...
test('RECEPTIONIST peut choisir un SALES_AGENT actif de la même agence',async()=>{
  ...
  assert.equal(response.status,201);assert.equal(response.body.assignedUserId,'20');assert.equal(response.body.createdById,'10');
});
