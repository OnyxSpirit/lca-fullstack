...
const migration=source('database/legacy-migrations/021_ged_business_archive.sql');

test('GED-AUTO-01/02 devis émis et idempotent',()=>{assert.match(quote,/archiveQuotation/);assert.match(ged,/quotation:\$\{id\}:issued:v2/);assert.match(migration,/UNIQUE INDEX uk_documents_source_key/)});
...
test('GED-AUTO-05 facture émise',()=>{assert.match(billing,/safelyArchive\(`invoice:\$\{invoiceId\}:issued`.*archiveInvoice/s);assert.match(ged,/invoice:\$\{id\}:issued:v2/)});
...
test('GED-AUTO-09/10 livraison finalisée et retry réparateur',()=>{assert.equal((delivery.match(/archiveDelivery\(id/g)??[]).length,2);assert.match(ged,/delivery:\$\{id\}:finalized:v2/)});
...
test('GED-RBAC-01..05 politiques dynamiques et agence conservées',()=>{const access=source('src/modules/documents/document-access.ts');assert.match(access,/POLICIES/);assert.match(access,/rbac\?\.permissions/);assert.doesNotMatch(access,/DIRECTOR|PARTS_MANAGER/);assert.match(routes,/documentAccessPredicate/);assert.match(access,/agency_id=\?/)});
