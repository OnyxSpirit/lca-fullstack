...
test('la migration Sales conserve les historiques et ajoute une unicité idempotente',()=>{const sql=readFileSync(new URL('../database/legacy-migrations/node-express/001_sales_stabilization.sql',import.meta.url),'utf8');assert.match(sql,/UNIQUE KEY uk_sales_idempotency/);assert.match(sql,/created_by/);assert.match(sql,/cancellation_reason/);assert.doesNotMatch(sql,/DELETE FROM sales|DROP TABLE sales/i)});
