...
test('SAV-OR-16 bloque l’annulation dès ready côté backend et masque PRET côté frontend',async()=>{
  ...
  assert.match(page,/\['PRET','FACTURE','LIVRE','CLOTURE'\]\.includes\(ro\.status\)/);
});
...
test('SAV-OR-18 sépare pointage opérationnel et main-d’œuvre financière',async()=>{
  ...
  assert.match(page,/Cette section ne crée aucune ligne facturable/);
  assert.match(page,/Seules ces lignes alimentent le récapitulatif financier et la facture/);
  assert.match(page,/Sans intervention liée/);
});
