...
export const mapRepair = (r: any): RepairOrder => ({
  ...
  approvals:(r.approvals??[]).map((x:any)=>({id:s(x.id),approved:Number(x.approved)===1,approvedAmount:x.approved_amount==null?null:n(x.approved_amount),customerName:x.customer_name,signatureData:x.signature_data??'',notes:x.notes??'',recordedByName:x.recorded_by_name??'',recordedAt:x.recorded_at})),
  customerApproval:{required:Boolean(r.customerApproval?.required),decided:Boolean(r.customerApproval?.decided),decision:r.customerApproval?.decision??null,canDecide:Boolean(r.customerApproval?.canDecide),submittedAmount:n(r.customerApproval?.submittedAmount)},
  ...
  financiallyCleared:Boolean(r.financially_cleared),
  workStarted:Boolean(r.workStarted),
  actualBillable:{expectedItems:n(r.actualBillable?.expectedItems),confirmedItems:n(r.actualBillable?.confirmedItems),confirmed:Boolean(r.actualBillable?.confirmed)},
  ...
  warranty:r.warranty?{id:s(r.warranty.id),version:n(r.warranty.version),coverageMode:r.warranty.coverage_mode,decisionStatus:r.warranty.decision_status,allocationStatus:r.warranty.allocation_status,providerId:r.warranty.provider_id?s(r.warranty.provider_id):null,providerName:r.warranty.provider_name??null,authorizationReference:r.warranty.authorization_reference??null,decisionComment:r.warranty.decision_comment??null,allocations:(r.warranty.allocations??[]).map((a:any)=>({repairOrderItemId:s(a.repair_order_item_id),manufacturerShareHT:n(a.manufacturer_share_ht),manufacturerTotal:n(a.manufacturer_total),customerTotal:n(a.customer_total),description:a.description,itemType:a.item_type,lineTotal:n(a.line_total),taxRate:n(a.tax_rate),itemStatus:a.item_status})),summary:{realTotal:n(r.warranty.summary?.realTotal),manufacturerTotal:n(r.warranty.summary?.manufacturerTotal),customerTotal:n(r.warranty.summary?.customerTotal)},claim:r.warranty.claim?{id:s(r.warranty.claim.id),claimNumber:r.warranty.claim.claim_number,status:r.warranty.claim.status,total:n(r.warranty.claim.total),amountReceived:n(r.warranty.claim.amount_received),balanceDue:n(r.warranty.claim.balance_due)}:null}:null,
...
export const useWarrantyProvidersQuery=(active=true)=>useQuery({queryKey:['warranty-providers'],queryFn:()=>apiRequest<Array<{id:string;code:string;name:string}>>('/warranty-providers'),enabled:enabled()&&active});
export const useWarrantyClaimPayment=()=>{const qc=useQueryClient();return useMutation({mutationFn:({claimId,...body}:{claimId:string;amount:number;reference?:string;requestKey:string})=>apiRequest(`/warranty-claims/${claimId}/payments`,{method:'POST',body:JSON.stringify(body)}),onSuccess:()=>{qc.invalidateQueries({queryKey:erpKeys.repairOrders});qc.invalidateQueries({queryKey:['reports','warranties']})}})};
