...
export const BillingPage: React.FC = () => {
  ...
  return <div className="space-y-6">
    ...
    <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-3">{[['CA facturé', money(billed)],...(canViewPayments?[['Encaissé', money(paid)], ['Reste dû', money(due)], ['Échu', money(overdue)], ['Taux encaissement', billed ? `${(paid / billed * 100).toFixed(1)} %` : '0 %']]:[])].map(([label, value]) => <Card key={label}><p className="text-xs text-slate-500">{label}</p><p className="text-xl font-bold mt-1">{value}</p></Card>)}</div>
