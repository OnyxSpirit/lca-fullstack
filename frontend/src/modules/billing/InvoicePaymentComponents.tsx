...
export const InvoicePaymentHistory:React.FC<{payments?:InvoicePayment[];canRefund?:boolean;onRefund?:(paymentId:string)=>void}>=({payments=[],canRefund=false,onRefund})=>{
 ...
 return payments.length?<div className="overflow-x-auto"><table className="w-full min-w-[720px] text-xs"><thead><tr className="text-left"><th>Règlement</th><th>Date</th><th>Méthode</th><th>Référence</th><th>Opérateur</th><th>Montant</th><th>Statut</th><th></th></tr></thead><tbody>{payments.map(item=><tr key={item.id} className="border-t"><td className="py-2 font-mono">{item.paymentNumber}</td><td>{formatDateTime(item.paymentDate)}</td><td>{item.paymentMethod}</td><td>{item.reference||'—'}</td><td>{item.receivedByName||'—'}</td><td>{formatCurrency(item.amount)}</td><td>{item.status}</td><td><Button size="xs" variant="outline" onClick={()=>void download(item)}>Reçu</Button>{canRefund&&item.status==='confirmed'&&onRefund&&<Button size="xs" variant="outline" onClick={()=>onRefund(item.id)}>Rembourser</Button>}</td></tr>)}</tbody></table></div>:<p className="text-sm text-slate-500">Aucun règlement.</p>;
};
