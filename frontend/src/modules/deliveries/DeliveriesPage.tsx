...
import { useDeliveriesQuery, useDeliveryCandidatesQuery, useDeliveryStatsQuery } from '../../api/erpHooks';
...
export const DeliveriesPage:React.FC=()=>{
  ...
  const [newOpen,setNewOpen]=useState(Boolean(requestedSaleId)&&canCreate),[search,setSearch]=useState(''),[status,setStatus]=useState(''),[date,setDate]=useState(new Date().toISOString().slice(0,10));
  const list=useDeliveriesQuery({search:useDeferredValue(search),status},canView),stats=useDeliveryStatsQuery(canView),deliveries=list.data??[];
  const candidatesQuery=useDeliveryCandidatesQuery(canCreate),candidates=candidatesQuery.data??[];
  ...
  return <div className="space-y-6">
    <PageHeader title="Livraisons véhicules" subtitle="Planification, préparation qualité, remise client et traçabilité." breadcrumbs={[{label:'Accueil',href:'/dashboard'},{label:'Livraisons'}]} actions={<div className="flex gap-2">{canCreate&&<Button size="sm" onClick={()=>setNewOpen(true)}>Planifier</Button>}<Button size="sm" variant="outline" icon={<Printer className="w-4 h-4"/>} onClick={()=>openDeliveryPlanningPdf(date).catch(value=>error('Planning indisponible',value))}>Planning PDF</Button></div>}/>
    ...
    {candidates.length>0&&<Card><div className="flex items-center justify-between gap-3"><div><h2 className="font-bold">Ventes à planifier</h2><p className="text-xs text-slate-500">{candidates.length} dossier(s) prêt(s) pour le planning de livraison.</p></div>{canCreate&&<Button size="sm" onClick={()=>setNewOpen(true)}>Planifier maintenant</Button>}</div></Card>}
