...
import{useCreateDelivery,useDeliveryCandidatesQuery,useDeliverySpecialistsQuery}from'../../api/erpHooks';
...
export const NewDeliveryModal:React.FC<{isOpen:boolean;onClose:()=>void;initialSaleId?:string}>=({isOpen,onClose,initialSaleId})=>{
  const auth=useAuthStore(),canSchedule=auth.can('delivery.schedule'),candidates=useDeliveryCandidatesQuery(isOpen&&canSchedule),create=useCreateDelivery(),addToast=useUiStore(s=>s.addToast);
  ...
  const specialistsQuery=useDeliverySpecialistsQuery(saleId,isOpen&&canSchedule),specialists=specialistsQuery.data??[],selectedSpecialist=specialists.find(user=>user.id===deliverySpecialistId);
  useEffect(()=>{if(isOpen)setSaleId(initialSaleId&&sales.some(s=>s.id===initialSaleId)?initialSaleId:(sales[0]?.id??''))},[isOpen,initialSaleId,candidates.data]);
  useEffect(()=>{setDeliverySpecialistId(current=>specialists.some(user=>user.id===current)?current:(specialists[0]?.id??''))},[saleId,specialistsQuery.data]);
  const submit=async(event:React.FormEvent)=>{event.preventDefault();if(!canSchedule||create.isPending||!selectedSpecialist)return;try{await create.mutateAsync({saleId,deliverySpecialistId,scheduledAt:new Date(scheduledAt).toISOString(),customerNotes:notes||undefined});addToast({type:'success',title:'Livraison planifiée',description:'Le Responsable livraison et la checklist ont été enregistrés.'});onClose()}catch(error){addToast({type:'error',title:'Planification impossible',description:error instanceof Error?error.message:'Erreur API'})}};
  return <Modal isOpen={isOpen} onClose={onClose} title="Planifier une livraison" description="Seules les ventes prêtes à livrer sont proposées." maxWidth="md"><form onSubmit={submit} className="space-y-4">
    ...
    <div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={onClose}>Annuler</Button><Button type="submit" disabled={!saleId||!selectedSpecialist||!scheduledAt} loading={create.isPending}>Planifier</Button></div>
  </form></Modal>;
};
