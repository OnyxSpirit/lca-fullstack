...
import { useCreateDelivery, useDeliveryCandidatesQuery } from '../../api/erpHooks';
...
export const NewDeliveryModal: React.FC<{isOpen:boolean;onClose:()=>void;initialSaleId?:string}> = ({ isOpen, onClose,initialSaleId }) => {
  const candidates=useDeliveryCandidatesQuery(),sales=(candidates.data??[]).map((row:any)=>({id:String(row.sale_id),saleNumber:row.sale_number,customerName:row.customer_name,vehicleLabel:row.vehicle_label}));
