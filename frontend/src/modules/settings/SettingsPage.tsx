...
const agencyFields = [
  { key: 'name', label: "Nom de l’agence", placeholder: "Saisir le nom de l’agence", required: true },
  { key: 'code', label: 'Code agence', placeholder: "Saisir le code de l’agence", required: true },
  { key: 'address', label: 'Adresse', placeholder: "Saisir l’adresse de l’agence", required: false },
  { key: 'city', label: 'Ville', placeholder: 'Saisir la ville', required: false },
  { key: 'phone', label: 'Téléphone', placeholder: 'Saisir le numéro de téléphone', required: false },
  { key: 'email', label: 'Adresse e-mail', placeholder: "Saisir l’adresse e-mail", required: false },
] as const;
...
export const SettingsPage: React.FC = () => {
  ...
  return <div className="space-y-6">
    ...
    {tab === 'agencies' && <div className="space-y-4">{admin&&<Card><CardHeader><div><CardTitle>Créer une agence</CardTitle><CardDescription>Les codes doivent être uniques dans la base.</CardDescription></div></CardHeader><form onSubmit={createAgency} className="grid gap-3 md:grid-cols-3">{agencyFields.map(({ key, label, placeholder, required }) => <label key={key} htmlFor={`create-agency-${key}`} className="text-xs font-semibold text-slate-700">{label}<input id={`create-agency-${key}`} required={required} className={`${field} mt-1`} placeholder={placeholder} value={String(agencyForm[key] ?? '')} onChange={e => setAgencyForm(current => ({ ...current, [key]: e.target.value }))} /></label>)}<div><Button type="submit" icon={<Plus className="h-4 w-4" />} loading={agencyActions.create.isPending}>Créer l’agence</Button></div></form></Card>}
      ...
      {admin&&editedAgency && <Card><CardHeader><div><CardTitle>Modifier {editedAgency.name}</CardTitle><CardDescription>L’historique métier reste rattaché à cette agence.</CardDescription></div></CardHeader><form onSubmit={saveAgency} className="grid gap-3 md:grid-cols-3">{agencyFields.map(({ key, label, placeholder, required }) => <label key={key} htmlFor={`edit-agency-${key}`} className="text-xs font-semibold text-slate-700">{label}<input id={`edit-agency-${key}`} required={required} className={`${field} mt-1`} placeholder={placeholder} value={String(editedAgency[key] ?? '')} onChange={e => setEditedAgency(current => current ? ({ ...current, [key]: e.target.value }) : current)} /></label>)}<div className="flex gap-2"><Button type="submit" loading={agencyActions.update.isPending}>Enregistrer</Button><Button type="button" variant="outline" onClick={() => setEditedAgency(null)}>Annuler</Button></div></form></Card>}
    </div>}
