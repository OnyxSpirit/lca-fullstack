...
const displayDate=(value:string|null)=>value?new Date(toLocalInput(value)).toLocaleString('fr-CG',{dateStyle:'short',timeStyle:'short'}):'—';
...
export const WorkshopPlanningPage:React.FC=()=>{
  ...
  const canView=can('workshop.view'),canViewPlanning=canView&&can('workshop.schedule.view'),canViewHistory=canView&&can('workshop.intervention.view'),canPlan=can('workshop.plan');
  const canAssignTechnician=can('workshop.assign_technician'),canAssignBay=can('workshop.assign_bay');
  const canViewTechnicians=can('workshop.technicians.view'),canViewBays=can('workshop.bay.view'),canViewProductivity=can('workshop.productivity.view');
  ...
  const error=planningQuery.error||historyQuery.error||techniciansQuery.error||baysQuery.error||statsQuery.error||unavailabilityQuery.error;
  ...
  const removeAbsence=async(item:TechnicianUnavailability)=>{if(removingAbsenceId===item.id||!window.confirm(`Supprimer l’indisponibilité « ${item.reason} » ?`))return;const trigger=document.activeElement instanceof HTMLButtonElement?document.activeElement:null;trigger?.setAttribute('disabled','');setRemovingAbsenceId(item.id);try{await absenceMutations.remove.mutateAsync({id:item.id,agencyId:agency?.id});addToast({type:'success',title:'Indisponibilité supprimée',description:'La capacité du technicien est rétablie.'})}catch(cause){addToast({type:'error',title:'Suppression impossible',description:cause instanceof Error?cause.message:'Erreur serveur'})}finally{trigger?.removeAttribute('disabled');setRemovingAbsenceId('')}};

  return <div className="space-y-5">
    <PageHeader title="Planning Atelier & Ponts" subtitle={`Vue ${mode==='day'?'journalière':'hebdomadaire'} — ${from}${to!==from?` au ${to}`:''}.`} breadcrumbs={[{label:'Accueil',href:'/dashboard'},{label:'Après-Vente'},{label:'Planning Atelier'}]} actions={<div className="flex gap-2">{canPlan&&canAssignTechnician&&<Button variant="outline" icon={<Clock className="w-4 h-4"/>} onClick={()=>{setAbsence({...absence,technicianId:technicians[0]?.id??''});setAbsenceOpen(true)}} disabled={!technicians.length}>Indisponibilité</Button>}{canPlan&&canAssignTechnician&&<Button icon={<Plus className="w-4 h-4"/>} onClick={openCreate} disabled={!technicians.length||!visibleRepairOrders.length}>Nouvelle affectation</Button>}</div>}/>
    {!canView&&<div role="alert" className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">Vous ne disposez pas de la permission workshop.view.</div>}
    ...
    {canViewProductivity&&<div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
    ...
    <Modal isOpen={Boolean(selected)} onClose={()=>{setSelected(null);setEditForm(null)}} title={`Affectation ${selected?.orderNumber??''}`} description={selected&&['completed','cancelled'].includes(selected.status)?'Affectation terminée — consultation uniquement':canPlan?'Modifier les ressources ou annuler uniquement cette réservation de planning.':'Consultation de l’affectation.'} maxWidth="xl">
      {selected&&effectiveEdit&&<form onSubmit={submitEdit} className="space-y-4">
        <div className="grid grid-cols-1 gap-3 rounded-lg bg-slate-50 p-3 text-sm sm:grid-cols-2"><div><span className="text-slate-500">Client</span><strong className="block">{selected.customerName||'—'}</strong></div><div><span className="text-slate-500">Véhicule</span><strong className="block">{selected.vehicleLabel||selected.registrationNumber||'—'}</strong></div><div><span className="text-slate-500">Intervention</span><strong className="block">{selected.interventionDescription||'Non précisée'}</strong></div><div><span className="text-slate-500">Statut</span><Badge variant="primary">{statusLabel[selected.status]??selected.status}</Badge></div></div>
        {canPlan&&!['completed','cancelled'].includes(selected.status)&&<div className="grid grid-cols-1 gap-3 sm:grid-cols-2">{canAssignTechnician&&<label className="text-xs font-semibold">Technicien<select required value={effectiveEdit.technicianId} onChange={event=>setEditForm({...effectiveEdit,technicianId:event.target.value})} className="mt-1 w-full rounded-md border p-2">{technicians.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label>}{canAssignBay&&<label className="text-xs font-semibold">Pont<select value={effectiveEdit.bayId} onChange={event=>setEditForm({...effectiveEdit,bayId:event.target.value})} className="mt-1 w-full rounded-md border p-2"><option value="">Sans pont</option>{bays.filter(item=>item.status==='available'||item.id===selected.bayId).map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label>}<label className="text-xs font-semibold">Début<input required type="datetime-local" value={effectiveEdit.startsAt} onChange={event=>setEditForm({...effectiveEdit,startsAt:event.target.value})} className="mt-1 w-full rounded-md border p-2"/></label><label className="text-xs font-semibold">Fin<input required type="datetime-local" value={effectiveEdit.endsAt} onChange={event=>setEditForm({...effectiveEdit,endsAt:event.target.value})} className="mt-1 w-full rounded-md border p-2"/></label></div>}
        <div className="flex flex-wrap justify-between gap-2 border-t pt-4">{canPlan&&!['completed','cancelled'].includes(selected.status)&&<Button type="button" variant="danger" loading={scheduleMutations.cancel.isPending} disabled={scheduleMutations.update.isPending} onClick={()=>void cancelSelected()}>Annuler l’affectation</Button>}<div className="flex gap-2"><Button type="button" variant="outline" icon={<ExternalLink className="w-4 h-4"/>} onClick={()=>navigate(`/service/repair-orders/${selected.repairOrderId}`)}>Consulter l’OR</Button>{canPlan&&canAssignTechnician&&!['completed','cancelled'].includes(selected.status)&&<Button type="submit" loading={scheduleMutations.update.isPending} disabled={scheduleMutations.cancel.isPending}>Enregistrer</Button>}</div></div>
      </form>}
    </Modal>
