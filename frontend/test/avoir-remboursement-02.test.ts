...
test('AR-FE-02 bornes paiement et avoir, idempotence et pending',()=>{assert.match(detail,/Math\.min\(refundPayment\?\.refundableRemaining/);assert.match(detail,/refundForm\.amount<=0/);assert.match(detail,/idempotencyKey:refundKey/);assert.match(detail,/if\(!refundPayment\|\|pending\)return/)});
