...
test('les modules visibles respectent la matrice des 13 rôles',()=>{
  const expected:Record<UserRole,string[]>={
    ...
    DIRECTION:['crm','customers','vehicles','showroom','sales','deliveries','service','workshop','parts','billing','documents','reports','users','settings'],
    ...
    WORKSHOP_CHIEF:['vehicles','service','workshop','parts','documents','reports','settings'],
    TECHNICIAN:['service','workshop','parts','documents'],
    PARTS_MANAGER:['service','workshop','parts','documents','reports','settings'],
...
test('les routes sensibles possèdent un garde alimenté par les permissions dynamiques',()=>{const app=readFileSync(new URL('../src/App.tsx',import.meta.url),'utf8'),store=readFileSync(new URL('../src/stores/authStore.ts',import.meta.url),'utf8');assert.match(app,/function ModuleGuard/);for(const module of ['users','billing','reports','documents'])assert.match(app,new RegExp(`ModuleGuard module="${module}"`));assert.match(app,/AccessDeniedPage/);assert.match(store,/hasDynamicPermission\(currentUser\.permissions/);assert.doesNotMatch(store,/canAccessModule\(currentUser\.roles/)});
...
test('Paramètres possède un garde admin et des hooks dédiés typés',()=>{const app=readFileSync(new URL('../src/App.tsx',import.meta.url),'utf8'),hooks=readFileSync(new URL('../src/api/settingHooks.ts',import.meta.url),'utf8');assert.match(app,/ModuleGuard module="settings"/);assert.match(hooks,/interface ConcessionSettings/);assert.match(hooks,/interface WorkshopLaborRate/);assert.doesNotMatch(hooks,/apiRequest<any>/);});
test('l’écran Paramètres ne simule aucune intégration et délègue les barèmes dynamiques',()=>{const page=readFileSync(new URL('../src/modules/settings/SettingsPage.tsx',import.meta.url),'utf8');assert.match(page,/WorkshopLaborRatesSettings/);assert.match(page,/Aucune intégration configurée/);assert.doesNotMatch(page,/Connecté|Certifié|SIRET|TVA Intracommunautaire/);assert.match(page,/Identifiant fiscal \/ NIU/);});
test('l’atelier sélectionne les barèmes backend dynamiques sans TVA codée en dur',()=>{const page=readFileSync(new URL('../src/modules/service/RepairOrderDetailPage.tsx',import.meta.url),'utf8');assert.match(page,/useRepairLaborRatesQuery/);assert.match(page,/laborRateId/);assert.match(page,/workshopConfig\?\.rates\.map/);assert.doesNotMatch(page,/taxRate:18\.9/);});
...
test('Dashboard et Portail utilisent exclusivement l’agrégat backend pour leurs KPI',()=>{const dashboard=readFileSync(new URL('../src/modules/dashboard/DashboardPage.tsx',import.meta.url),'utf8'),portal=readFileSync(new URL('../src/modules/modules-portal/ModulesPortalPage.tsx',import.meta.url),'utf8');assert.match(dashboard,/useDashboardOverviewQuery/);assert.match(portal,/useDashboardOverviewQuery/);for(const source of [dashboard,portal])for(const fake of ['+12.5%','4 essais prévus cette semaine','Stable','Mercredi','+14%','8 Actifs','20 Véhicules','3 Visiteurs','2 Prévues','3 En cours','0.085'])assert.doesNotMatch(source,new RegExp(fake.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')));});
test('les événements métier invalident l’agrégat Dashboard',()=>{const bootstrap=readFileSync(new URL('../src/components/AppBootstrap.tsx',import.meta.url),'utf8');assert.match(bootstrap,/dashboardOverviewKey/);assert.match(bootstrap,/invalidateQueries\(\{queryKey:dashboardOverviewKey\}\)/);});
