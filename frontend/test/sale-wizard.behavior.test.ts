...
test('SALE-DETAIL-03 billing.payment.collect obtient le chemin Enregistrer un règlement',async()=>{const{readFile}=await import('node:fs/promises'),source=await readFile(new URL('../src/modules/sales/SaleDetailPage.tsx',import.meta.url),'utf8');assert.match(source,/can\('billing\.payment\.collect'\)/);assert.match(source,/canPay&&sale\.remainingBalanceTTC>0\?'Enregistrer un règlement'/)});
test('SALE-DETAIL-04 sans billing.payment.collect la consultation reste disponible sans action interdite',async()=>{const{readFile}=await import('node:fs/promises'),source=await readFile(new URL('../src/modules/sales/SaleDetailPage.tsx',import.meta.url),'utf8');assert.match(source,/'Voir la facture et les règlements'/);assert.match(source,/encaissement doit être effectué par la comptabilité/)});
